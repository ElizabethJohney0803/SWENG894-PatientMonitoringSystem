...
class TestPatientDoctorAssignmentIntegration:
    ...
    def test_multiple_patients_single_doctor_workflow(self):
        ...
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC456"
        )
    ...
    def test_patient_reassignment_workflow(self):
        ...
        doctor1_user = User.objects.create_user(
            username="doctor1_reassign", password="pass"
        )
        doctor1_profile = UserProfile.objects.create(
            user=doctor1_user, role="doctor", license_number="DOC111"
        )

        doctor2_user = User.objects.create_user(
            username="doctor2_reassign", password="pass"
        )
        doctor2_profile = UserProfile.objects.create(
            user=doctor2_user, role="doctor", license_number="DOC222"
        )

        # Create patient
        patient_user = User.objects.create_user(
            username="patient_reassign", password="pass"
        )
        patient_profile = UserProfile.objects.create(user=patient_user, role="patient")
        patient = patient_profile.patient_record
    ...
    def test_doctor_deletion_patient_preservation(self):
        ...
        doctor_user = User.objects.create_user(
            username="doctor_delete", password="pass"
        )
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC789"
        )

        patient_user = User.objects.create_user(
            username="patient_preserve", password="pass"
        )
        patient_profile = UserProfile.objects.create(user=patient_user, role="patient")
        patient = patient_profile.patient_record
    ...
    def test_admin_interface_integration(self):
        ...
        # Create users
        admin_user = User.objects.create_user(
            username="admin_interface", password="pass"
        )
        admin_profile = UserProfile.objects.create(user=admin_user, role="admin")

        doctor_user = User.objects.create_user(
            username="doctor_interface",
            first_name="Interface",
            last_name="Doctor",
            password="pass",
        )
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC999"
        )

        patient_user = User.objects.create_user(
            username="patient_interface", password="pass"
        )
        patient_profile = UserProfile.objects.create(user=patient_user, role="patient")
        patient = patient_profile.patient_record
    ...
    def test_permission_integration_across_roles(self):
        ...
        for role in roles:
            ...
            if role in ["doctor", "nurse", "pharmacy"]:
                profile = UserProfile.objects.create(
                    user=user, role=role, license_number=f"{role.upper()}123"
                )
            else:
        ...
        for role, user in users.items():
            ...
            elif role in ["admin", "pharmacy"]:
                # These roles see all patients
                assert patient in queryset
            elif role == "nurse":
                # Nurse sees only their assigned patients; this nurse has none
    ...
    def test_validation_and_constraints_integration(self):
        ...
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC555"
        )
        ...
        patient_user = User.objects.create_user(
            username="patient_valid", password="pass"
        )
        patient_profile = UserProfile.objects.create(user=patient_user, role="patient")
        patient = patient_profile.patient_record
    ...
    def test_bulk_operations_integration(self):
        ...
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC_BULK"
        )
        ...
        for i in range(5):
            user = User.objects.create_user(
                username=f"patient_bulk_{i}", password="pass"
            )
            profile = UserProfile.objects.create(user=user, role="patient")
            patients.append(profile.patient_record)
    ...
    def test_emergency_contact_access_integration(self):
        ...
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC_EC"
        )
...
class TestPatientDoctorAssignmentPerformance:
    ...
    def test_queryset_performance_with_many_patients(self):
        ...
        doctor_profile = UserProfile.objects.create(
            user=doctor_user, role="doctor", license_number="DOC_PERF"
        )
        ...
        for i in range(50):  # Reduced for test efficiency
            user = User.objects.create_user(
                username=f"patient_perf_{i}", password="pass"
            )
            profile = UserProfile.objects.create(user=user, role="patient")
